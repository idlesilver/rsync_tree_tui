...
`--permission-group` / `RSYNC_TREE_TUI_PERMISSION_GROUP` 提供一个可选 selected group；只有选择 `grp:*` 且弹窗中 group 不是 `not change group` 时才会执行 `chgrp`。`any:*` 不依赖任何特殊 group，而是直接放开 other 权限。
...
```bash
# 远程端：发布后的数据集只允许浏览和下载
bash /tmp/setup_remote_permissions.sh any:r /remote/storage/datasets/v1.0

# 远程端：开放 staging 目录，允许团队上传
bash /tmp/setup_remote_permissions.sh --group asset_team grp:w /remote/storage/datasets/staging
