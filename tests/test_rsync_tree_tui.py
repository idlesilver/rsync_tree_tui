...
class ConfigTests(unittest.TestCase):
    ...
    def test_dotenv_indexed_local_without_matching_remote_uses_default(self) -> None:
        config_path = Path(self.tmp.name) / "config.json"
        project_dir = Path("project")
        project_dir.mkdir()
        env_file = project_dir / ".env"
        env_file.write_text(
            ...
            "RSYNC_TREE_TUI_REMOTE=default:/remote/project\n"
        ...
        self.assertEqual(config.remote_spec, "default:/remote/project")

    def test_shell_remote_overrides_indexed_remotes_but_keeps_indexed_locals(self) -> None:
        config_path = Path(self.tmp.name) / "config.json"
        project_dir = Path("project")
        project_dir.mkdir()
        env_file = project_dir / ".env"
        ...
        os.environ["RSYNC_TREE_TUI_REMOTE"] = "shell:/remote/project"
        args = argparse.Namespace(
            local_root=None,
            remote=None,
            permission_group=None,
        ...
        self.assertEqual(config.remote_spec, "shell:/remote/project")
    ...
    def test_dotenv_indexed_remote_without_local_root_raises_error(self) -> None:
        config_path = Path(self.tmp.name) / "config.json"
        env_file = Path(".env")
        env_file.write_text("RSYNC_TREE_TUI_REMOTE_1=one:/remote/project\n")
        args = argparse.Namespace(
            local_root=None,
            remote=None,
            permission_group=None,
    ...
    def test_dotenv_indexed_local_without_remote_raises_error(self) -> None:
        config_path = Path(self.tmp.name) / "config.json"
        env_file = Path(".env")
        env_file.write_text("RSYNC_TREE_TUI_LOCAL_ROOT_1=./local-one\n")
        args = argparse.Namespace(
            local_root=None,
            remote=None,
            permission_group=None,
        ...
        with self.assertRaisesRegex(
            ValueError,
            "RSYNC_TREE_TUI_REMOTE_1.*RSYNC_TREE_TUI_REMOTE",
        ):
            tui.resolve_app_config(args)
    ...
    def test_install_remote_update_rejects_payload_version_mismatch(self) -> None:
        with (
            mock.patch(
                ...
                return_value=self.remote_source("0.2.8"),
            ),
...
class MouseTests(unittest.TestCase):
    ...
    def test_mouse_wheel_moves_one_row(self) -> None:
        app = self.make_app_with_nodes()
    ...
    def test_footer_shortcuts_drop_left_groups_and_keep_helper_when_narrow(self) -> None:
        app = self.make_app_with_nodes()
        app.stdscr = mock.Mock()

        with mock.patch("rsync_tree_tui.curses.color_pair", side_effect=lambda n: n * 100):
            app._render_footer_shortcuts(y=20, width=62)

        rendered = "".join(call.args[2] for call in app.stdscr.addnstr.call_args_list)
        self.assertNotIn("Up/Down", rendered)
        self.assertNotIn("Left/Right", rendered)
        self.assertNotIn("Space", rendered)
        self.assertIn("?=helper", rendered)
        self.assertIn("p/P=permission", rendered)
        hit_keys = {hit.key for hit in app.footer_shortcut_hits}
        self.assertIn(ord("?"), hit_keys)
        self.assertIn(ord("p"), hit_keys)
        self.assertIn(ord("P"), hit_keys)
